...
<script type="text/javascript">
	...
	$(function(){
		...
		$("#getbundModal").click(function (){
			$("#bundModal").modal("show");
			$("#search-activitys").val("");
		})

		$("#search-activitys").keydown(function (even){
			if(even.keyCode==13){
				//开始执行搜索行为
				searchactivity();
				return false;
		...
		$("#relevance").click(function (){
			...
			if($xz.length==0){
				...
				$.ajax({
					...
					url : "workbench/clue/relevance.do",
					data : parm,
					type : "post",
					dataType : "json",
	...
	function searchactivity(){
		var name=$("#search-activitys").val();
		var html="";
		var id="${c.id}";
		$.ajax({

			url : "workbench/clue/seachactivity.do",
			data : {
				"name":name,
				'clueId': id
			},
			...
			success : function (data) {
				$.each(data,function (i,n){
					if(n!=null){
						...
						html+='	<td>'+n.endDate+'</td>';
						html+='	<td>'+n.owner+'</td>';
						html+='	<td><a href="javascript:void(0);" onclick="delRelation(\''+id+'\',\''+n.id+'\')" style="text-decoration: none;"><span class="glyphicon glyphicon-remove"></span>解除关联</a></td>';
						html+='	</tr>';
					}
				})
	...
	//解除关联市场活动
	function delRelation(clueId,activityId){
		if(confirm("确定解除吗？")){
			$.ajax({
				url : "workbench/clue/delRelation.do",
				data : {
					'clueId':clueId,
...
<body>
		...
		<div class="modal-dialog" role="document" style="width: 80%;">
			<div class="modal-content">
					...
					<div class="btn-group" style="position: relative; top: 18%; left: 8px;">
						<form class="form-inline" role="form">
						  <div class="form-group has-feedback">
						    <input id="search-activitys" type="text" class="form-control" style="width: 300px;" placeholder="请输入市场活动名称，支持模糊查询">
				...
				<div class="modal-footer">
					...
					<button id="relevance" type="button" class="btn btn-primary" <%--data-dismiss="modal"--%>>关联</button>
				</div>
			</div>
		</div>
	</div>
	...
	<div>
		<div style="position: relative; top: 60px; left: 40px;">
			...
			<div>
				<a id="getbundModal" href="javascript:void(0);" style="text-decoration: none;"><span class="glyphicon glyphicon-plus"></span>关联市场活动</a>
			</div>
		</div>
	</div>
